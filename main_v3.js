...
/* ── 10. CONTINUOUS WIND EMITTER ENGINE (V3) ── */
(function initCanvasTransition() {
  ...
  let width, height;
  let particles = [];
  const MAX_SMOKE = 200; // Less dense
  const MAX_DUST = 250; 
  ...
  spriteImage.onload = () => {
    ...
    tintedSprite = offCanvas; 
    
    window.addEventListener('resize', resize);
    resize();
    
    // Pre-warm the system so the screen isn't empty on load
    for(let i=0; i<80; i++) spawnSmoke(true); // Less pre-warm
    for(let i=0; i<100; i++) spawnDust(true);
  ...
  function spawnSmoke(preWarm = false) {
    const startX = preWarm ? Math.random() * width : -200 - Math.random() * 100;
    particles.push({
      type: 'smoke',
      x: startX, 
      y: viewportHeight * 0.55 + (Math.random() - 0.5) * (viewportHeight * 0.05), // Narrower vertical spawn
      vx: Math.random() * 0.5 + 0.2, // Slowed down significantly
      vy: (Math.random() - 0.5) * 0.2,
      // Smoke expands as it travels. Base size is small.
      baseSize: Math.random() * 100 + 50, // Smaller base size to keep stream narrower
      size: 0, 
      opacity: preWarm ? (Math.random() * 0.15 + 0.05) : 0, // Lowered opacity so text is readable
      targetOpacity: Math.random() * 0.15 + 0.05, 
      rotation: Math.random() * Math.PI * 2,
      rotationSpeed: (Math.random() - 0.5) * 0.002,
      baseVx: Math.random() * 0.5 + 0.2, // Slowed down significantly
      baseVy: (Math.random() - 0.5) * 0.1
    });
  }

  function spawnDust(preWarm = false) {
    let onScreenSmoke = particles.filter(p => p.type === 'smoke' && p.x > -100 && p.x < width + 100);
    if (onScreenSmoke.length === 0) return;
    
    const sp = onScreenSmoke[Math.floor(Math.random() * onScreenSmoke.length)];
    
    particles.push({
      type: 'dust',
      x: sp.x + (Math.random() - 0.5) * sp.size * 0.5, 
      y: sp.y + (Math.random() - 0.5) * sp.size * 0.3 + (preWarm ? Math.random() * 500 : 0),
      vx: sp.vx * 0.8 + (Math.random() - 0.5) * 1.5,
      vy: Math.random() * 0.5 + 0.1, 
      size: Math.random() * 2 + 1,
      opacity: Math.random() * 0.8 + 0.2,
      rotation: Math.random() * Math.PI,
      rotationSpeed: (Math.random() - 0.5) * 0.1, 
      baseVx: (Math.random() - 0.5) * 0.5 + sp.vx * 0.5,
      baseVy: Math.random() * 1.5 + 0.5
    });
  }
  ...
  function animate() {
    ...
    smokeSpawnTimer++;
    if (smokeSpawnTimer > 6) { // Slower spawn rate
      smokeSpawnTimer = 0;
      let currentSmoke = particles.filter(p => p.type === 'smoke').length;
      if (currentSmoke < MAX_SMOKE) spawnSmoke();
    }

    dustSpawnTimer++;
    if (dustSpawnTimer > 3) { 
      dustSpawnTimer = 0;
      let currentDust = particles.filter(p => p.type === 'dust').length;
      if (currentDust < MAX_DUST) {
        spawnDust();
      }
    }
    ...
    for (let i = particles.length - 1; i >= 0; i--) {
      ...
      if (p.type === 'smoke') {
        const progressX = Math.max(0, p.x / width);
        p.size = p.baseSize + (progressX * 150); // Less expansion to keep it narrow
        const turbulence = Math.sin(globalTime + p.x * 0.005) * 0.2; // Weaker turbulence
        p.vy += (p.baseVy + turbulence - p.vy) * 0.02;
      } else {
      ...
      if (p.type === 'smoke') {
        if (p.opacity < p.targetOpacity) p.opacity += 0.002;
        const progressX = Math.max(0, p.x / width); 
        drawOpacity = p.opacity * Math.max(0, 1 - progressX * 1.2); 

        if (p.x > width + 200 || (p.x > 100 && drawOpacity <= 0.005)) {
        ...
        if (p.y > fadeEnd || drawOpacity <= 0.01) {
          particles.splice(i, 1);
          continue;
        }
      }
      ...
      if (p.type === 'smoke' && tintedSprite && drawOpacity > 0.005) {
        ctx.save();
        ...
        ctx.translate(p.x, p.y);
        ctx.rotate(p.rotation);
        ctx.fillStyle = `rgba(36, 138, 204, ${drawOpacity})`;
        ctx.beginPath();
        ...
        ctx.lineTo(p.x, p.y);
        ctx.strokeStyle = `rgba(36, 138, 204, ${drawOpacity * 0.5})`;
